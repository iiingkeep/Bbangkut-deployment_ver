...
const Community = ({loggedIn, baseURL}) => {                                                                     // 게시물 목록을 페이지별로 출력하는 컴포넌트
  ...
  const getPostContentWithoutStylesAndImages = (content) => {                                           // 게시글 내용에서 사진과 스타일 제외
        ...
        {posts.map((post) => (
          <li key={post.postid} className='commu-post-list'>
            ...
            <Link to={`/Community/Read/${post.postid}`}>
              ...
              <div className='commu-post-list__content' dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(getPostContentWithoutStylesAndImages(post.content)) }}></div>
            </Link>
            </div>
